defmodule Mix.Tasks.Gradient do
  ...
  defp filter_paths_for_app(app_name, app_files) do
    ...
    if file_overrides_enabled? do
      magic_comment = if enabled?, do: "# gradient:disable-for-file", else: "# gradient:enable-for-file"
  ...
  defp filter_files_with_magic_comment(app_name, beam_files, comment, should_include?) do
    ...
    Enum.map(beam_files, &ex_filename_from_beam/1)
    # Turn the relative path into absolute
    |> Enum.map(&Path.expand(&1, app_path))
    # Filter out any paths representing code generated by deps
    |> Enum.filter(fn filename -> not String.starts_with?(filename, deps_path) end)
  ...
  defp ex_filename_from_beam(file_path) do
    # May be charlist, convert to string
    file_path = to_string(file_path)

    if String.ends_with?(file_path, ".ex") or String.ends_with?(file_path, ".exs") do
      file_path
    else
      get_compiled_source_file(file_path)
    end
  end

  defp get_compiled_source_file(beam_path) do
